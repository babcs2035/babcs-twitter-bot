        ...
        for tweet in timeline:
            ...
            if len(tweetSplited) == 4:
                ...
                if tweetSplited[1] == "setFlag_atcoder" and (tweetSplited[3] == "on" or tweetSplited[3] == "off"):
                    ...
                    tweetText += AtCoder.detection.setFlag(
            ...
            if len(tweetSplited) >= 3:
                ...
                if tweetSplited[1] == "reg_atcoder":
                    tweetText = AtCoder.register.register(
                ...
                if tweetSplited[1] == "reg_aoj":
                    tweetText = AOJ.register.register(
                        tweetSplited[2], str(userData["screen_name"]), 0)
                    api.update_status(tweetText + timeStamp,
                                      in_reply_to_status_id=tweet["id"])
                    log.logger.info("cper_bot-twitter: Tweeted " + tweetText)
                ...
                if tweetSplited[1] == "reg_cf":
                    tweetText = CF.register.register(
                        tweetSplited[2], str(userData["screen_name"]), 0)
                    api.update_status(tweetText + timeStamp,
                                      in_reply_to_status_id=tweet["id"])
                    log.logger.info("cper_bot-twitter: Tweeted " + tweetText)
                ...
                if tweetSplited[1] == "reg_yk":
                    tweetText = YK.register.register(
                        tweetSplited[2], str(userData["screen_name"]), 0)
                    api.update_status(tweetText + timeStamp,
                                      in_reply_to_status_id=tweet["id"])
                    log.logger.info("cper_bot-twitter: Tweeted " + tweetText)
