...
def status(atcoderID):
    ...
    if atcoderID in acCount:
        tweetText += "今日の Unique AC 数 : " + \
            str(int(userData["accepted_count"]) -
                int(acCount[atcoderID])) + "\n"
    if atcoderID in acPoint:
        tweetText += "今日の Unique AC の Rated Point Sum : " + \
            str(int(userData["rated_point_sum"]) -
                int(acPoint[atcoderID])) + "\n"
    if atcoderID in acCount and atcoderID in acPoint:
        if int(userData["accepted_count"]) - int(acCount[atcoderID]) > 0:
            tweetText += "今日の Rated Point Sum / Unique AC 数 : " + str((int(userData["rated_point_sum"]) - int(
                acPoint[atcoderID])) / (int(userData["accepted_count"]) - int(acCount[atcoderID]))) + "\n"
    else:
