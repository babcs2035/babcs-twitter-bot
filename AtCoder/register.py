...
def checkAtCoderID(atcoderID):
    ...
    try:
        ...
        print("cper_bot-AtCoder-register: " + atcoderID + " is correct AtCoder ID")
        return True
    except:
        print("cper_bot-AtCoder-register: " + atcoderID + " is not correct AtCoder ID")
        return False
...
def register(atcoderID, twitterID, flag):
    ...
    print("cper_bot-AtCoder-register: ----- AtCoder-register Start -----")
    ...
    if flag == 0:
        if checkAtCoderID(atcoderID):
            if (atcoderID, twitterID) not in AtCoderIDs:
                ...
                print("cper_bot-AtCoder-register: Registered new AtCoder ID : " + atcoderID)
                AtCoderIDsFixedFlag = True
            else:
                tweetText = "@" + str(twitterID) + " この AtCoder ID は既に登録されています！\n"
                print("cper_bot-AtCoder-register: Rejected to register AtCoder ID : " + atcoderID)
        else:
            tweetText = "@" + str(twitterID) + " 正しい AtCoder ID ではありません！\n"
            print("cper_bot-AtCoder-register: Rejected to AtCoder-register new AtCoder ID : " + atcoderID)
    if flag == 1:
        if checkAtCoderID(atcoderID):
            if (atcoderID, twitterID) in AtCoderIDs:
                AtCoderIDs.remove((atcoderID, twitterID))
                tweetText = "@" + str(twitterID) + " AtCoder ID を登録解除しました！\n"
                print("cper_bot-AtCoder-register: Unregistered AtCoder ID : " + atcoderID)
                AtCoderIDsFixedFlag = True
            else:
                tweetText = "@" + str(twitterID) + " この AtCoder ID は登録されていません！\n"
                print("cper_bot-AtCoder-register: Rejected to unregister AtCoder ID : " + atcoderID)
        else:
            tweetText = "@" + str(twitterID) + " 正しい AtCoder ID ではありません！\n"
            print("cper_bot-AtCoder-register: Rejected to unregister AtCoder ID : " + atcoderID)
    ...
    print("cper_bot-AtCoder-register: ----- AtCoder-register End -----")
    return tweetText
