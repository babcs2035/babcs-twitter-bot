...
def checkAtCoderID(atcoderID):
    ...
    try:
        ...
        print("cpcontest_bot-register: " + atcoderID + " is correct AtCoder ID")
        return True
    except:
        print("cpcontest_bot-register: " + atcoderID + " is not correct AtCoder ID")
        return False
...
def uploadToDropbox():
    ...
    if AtCoderIDsFixedFlag:
        ...
        print("cpcontest_bot-register: Uploaded AtCoderIDs (size : ", str(len(AtCoderIDs)), ")")
    
# AtCoder ID 登録・解除
def register(atcoderID, twitterID, flag):

    print("cpcontest_bot-register: ----- register Start -----")
    ...
    if flag == 0:
        if checkAtCoderID(atcoderID):
            if (atcoderID, twitterID) not in AtCoderIDs:
                ...
                print("cpcontest_bot-register: Registered new AtCoder ID : " + atcoderID)
                AtCoderIDsFixedFlag = True
            else:
                tweetText = "@" + str(twitterID) + " この AtCoder ID は既に登録されています！\n"
                print("cpcontest_bot-register: Rejected to register AtCoder ID : " + atcoderID)
        else:
            tweetText = "@" + str(twitterID) + " 正しい AtCoder ID ではありません！\n"
            print("cpcontest_bot-register: Rejected to register new AtCoder ID : " + atcoderID)
    if flag == 1:
        if checkAtCoderID(atcoderID):
            if (atcoderID, twitterID) in AtCoderIDs:
                AtCoderIDs.remove((atcoderID, twitterID))
                tweetText = "@" + str(twitterID) + " AtCoder ID を登録解除しました！\n"
                print("cpcontest_bot-register: Unregistered AtCoder ID : " + atcoderID)
                AtCoderIDsFixedFlag = True
            else:
                tweetText = "@" + str(twitterID) + " この AtCoder ID は登録されていません！\n"
                print("cpcontest_bot-register: Rejected to unregister AtCoder ID : " + atcoderID)
        else:
            tweetText = "@" + str(twitterID) + " 正しい AtCoder ID ではありません！\n"
            print("cpcontest_bot-register: Rejected to unregister AtCoder ID : " + atcoderID)
    ...
    print("cpcontest_bot-register: ----- register End -----")
    return tweetText
