...
def downloadFromDropbox():
    ...
    dbx.files_download_to_file(
        "cpcontest_bot/FAFlags.txt", "/cpcontest_bot/FAFlags.txt")
    with open("cpcontest_bot/FAFlags.txt", "rb") as f:
        FAFlags = pickle.load(f)
    log.logger.info(
        "cpcontest_bot-FA: Downloaded FAFlags (size : ", str(len(FAFlags)), ")")

    # rankings をダウンロード
    dbx.files_download_to_file(
        "cpcontest_bot/rankings.txt", "/cpcontest_bot/rankings.txt")
    with open("cpcontest_bot/rankings.txt", "rb") as f:
        rankings = pickle.load(f)
    log.logger.info(
        "cpcontest_bot-ranking: Downloaded rankings (size : ", str(len(rankings)), ")")

    # scores をダウンロード
    dbx.files_download_to_file(
        "cpcontest_bot/scores.txt", "/cpcontest_bot/scores.txt")
    with open("cpcontest_bot/scores.txt", "rb") as f:
        scores = pickle.load(f)
    log.logger.info(
        "cpcontest_bot-updateHighestScore: Downloaded scores (size : ", str(len(scores)), ")")
    ...
    log.logger.info(
        "cpcontest_bot-ranking: Downloaded AtCoderIDs (size : ", str(len(AtCoderIDs)), ")")
...
def uploadToDropbox():
    ...
    log.logger.info(
        "cpcontest_bot-FA: Uploaded FAFlags (size : ", str(len(FAFlags)), ")")
    ...
    with open("cpcontest_bot/rankings.txt", "rb") as f:
        dbx.files_upload(f.read(), "/cpcontest_bot/rankings.txt",
                         mode=dropbox.files.WriteMode.overwrite)
    log.logger.info(
        "cpcontest_bot-ranking: Uploaded rankings (size : ", str(len(rankings)), ")")
    ...
    with open("cpcontest_bot/scores.txt", "rb") as f:
        dbx.files_upload(f.read(), "/cpcontest_bot/scores.txt",
                         mode=dropbox.files.WriteMode.overwrite)
    log.logger.info(
        "cpcontest_bot-updateHighestScore: Uploaded scores (size : ", str(len(scores)), ")")
...
def downloadImage(url, dst_path):
    ...
    try:
        ...
        log.logger.info("cpcontest_bot-ranking: downloadImage Error (url = " +
...
def cpcontest(contests):
    ...
    for contest in contests:
        ...
        try:
            ...
            log.logger.info("cpcontest_bot-FA: topHTML Error")
            break
        contestName = str(
            topData.contents[3].contents[1].contents[1].contents[0])[0:-10]

        # 順位表 json データを取得
        request = session.get(url="https://atcoder.jp/contests/" + str(
            contest) + "/standings" + ("/team" if teamFlag else "") + "/json")
        try:
            standingsJsonData = json.loads(request.text)
            log.logger.info("cpcontest_bot-FA: Downloaded standingsJsonData")
        except:
            log.logger.info("cpcontest_bot-FA: standingsJsonData Error")
            break

        # 順位表から最高得点を見つける
        for task in standingsJsonData["TaskInfo"]:
            if task["TaskScreenName"] + ("_team" if teamFlag else "") not in scores:
                scores[task["TaskScreenName"] +
                       ("_team" if teamFlag else "")] = 0
        taskIndex = 0
        for task in standingsJsonData["TaskInfo"]:
            maxScore = -1
            minTime = "-1"
            ...
            if maxScore > scores[task["TaskScreenName"] + ("_team" if teamFlag else "")]:
                ...
                for atcoderID, twitterID in AtCoderIDs:
                    if atcoderID == maxUser:
                        ...
                        break
                scores[task["TaskScreenName"] +
                       ("_team" if teamFlag else "")] = maxScore
                ...
                if isUserTeam:
                    ...
                    maxUser = maxUser + \
                        (" ( @" + userTwitterID + " ) " if flag else " ")
                api.update_status("〔" + contestName + ("（チーム戦）" if teamFlag else "") + " 実況〕\n" + maxUser + "さんが " + assign + " 問題で " + str(
                    maxScore) + " 点を " + minTime + " に獲得し，最高得点を更新しました！\nhttps://atcoder.jp/contests/" + contest + "/standings\n" + timeStamp)
                log.logger.info("cpcontest_bot-updateHighestScore: detected " + str(task["TaskScreenName"] + (
                    "_team" if teamFlag else "")) + " updated the highest score (" + maxUser + ")")
            taskIndex += 1

        # 順位表から FA を見つける
        for task in standingsJsonData["TaskInfo"]:
            if task["TaskScreenName"] + ("_team" if teamFlag else "") not in FAFlags:
                FAFlags[task["TaskScreenName"] +
                        ("_team" if teamFlag else "")] = False
        for task in standingsJsonData["TaskInfo"]:
            ...
            if minTime != -1:
                ...
                if isUserTeam:
                    ...
                    minUser = " " + minUser + \
                        (" ( @" + userTwitterID + " ) " if flag else " ")
                api.update_status("〔" + contestName + ("（チーム戦）" if teamFlag else "") + " 実況〕\n" + task["Assignment"] + " 問題の FA を " + str(
                    sec_to_time(minTime)) + " で" + minUser + "さんが獲得しました！\nhttps://atcoder.jp/contests/" + contest + "/standings\n" + timeStamp)
                log.logger.info("cpcontest_bot-FA: detected " + str(task["TaskScreenName"] + (
                    "_team" if teamFlag else "")) + " FA (" + minUser + ")")
                ...
                if rows["UserScreenName"] in rankings[contest + ("_team" if teamFlag else "")]:
                    ...
                    if rankings[contest + ("_team" if teamFlag else "")][rows["UserScreenName"]] > rankNum and (rankNum <= 10 or flag):
                        ...
                        userpage = requests.get(
                            url="https://atcoder.jp/users/" + rows["UserScreenName"])
                        try:
                            userpage.raise_for_status()
                            userpageData = BeautifulSoup(
                                userpage.text, "html.parser")
                        except:
                            log.logger.info(
                                "cpcontest_bot-ranking: userpageData Error (user_screen_name = " + rows["UserScreenName"] + ")")

                        # 投稿する画像を作成
                        succeedFlag = True
                        try:
                            downloadImage(("aaa" if rows["IsTeam"] else userpageData.find(
                                "img", class_="avatar").attrs["src"]), "cpcontest_bot/data/" + rows["UserScreenName"] + ".png")
                            font = ImageFont.truetype(
                                "cpcontest_bot/data/fontR.ttc", 96)
                            fontB = ImageFont.truetype(
                                "cpcontest_bot/data/fontB.ttc", 96)
                            resImg = Image.new(
                                "RGB", (1405, 562), (255, 255, 255))
                            resDraw = ImageDraw.Draw(resImg)
                            resImg.paste(Image.open(
                                "cpcontest_bot/data/" + rows["UserScreenName"] + ".png").resize((512, 512)), (25, 25))
                            resDraw.text((567, 165), str(
                                rankings[contest + ("_team" if teamFlag else "")][rows["UserScreenName"]]) + " 位 ⇒", fill=(64, 64, 64), font=font)
                            resDraw.text((617, 281), str(rankNum) + " 位 (" + str(int(rankings[contest + (
                                "_team" if teamFlag else "")][rows["UserScreenName"]]) - int(rankNum)) + " 位 UP)", fill=(255, 64, 64), font=fontB)
                        ...
                        if succeedFlag:
                            api.update_status_with_media(filename="cpcontest_bot/data/resImg.jpg", status="〔" + contestName + ("（チーム戦）" if teamFlag else "") + " 実況〕\n" + userName + "さんが " + str(
                                rankings[contest + ("_team" if teamFlag else "")][rows["UserScreenName"]]) + " 位から " + str(rankNum) + " 位に浮上しました！\nhttps://atcoder.jp/contests/" + contest + "/standings\n" + timeStamp)
                        else:
                            api.update_status("〔" + contestName + ("（チーム戦）" if teamFlag else "") + " 実況〕\n" + userName + "さんが " + str(rankings[contest + (
                                "_team" if teamFlag else "")][rows["UserScreenName"]]) + " 位から " + str(rankNum) + " 位に浮上しました！\nhttps://atcoder.jp/contests/" + contest + "/standings\n" + timeStamp)
                        log.logger.info("cpcontest_bot-ranking: detected ranking updated (" +
                                        rows["UserScreenName"] + ("_team" if teamFlag else "") + ")")
