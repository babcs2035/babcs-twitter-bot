...
def downloadFromDropbox():
    ...
    print("cper_bot-AOJ-ranking: Downloaded AOJIDs (size : ", str(len(AOJIDs)), ")")
...
def uploadToDropbox():
    ...
    with open("AOJ/acCount.txt", "rb") as f:
        dbx.files_upload(f.read(), "/AOJ/acCount.txt", mode = dropbox.files.WriteMode.overwrite)
...
def ranking():
    ...
    for (AOJID, twitterID) in AOJIDs:
        acCountJson = urllib.request.urlopen("https://judgeapi.u-aizu.ac.jp/users/" + str(AOJID))
    ...
    for (AOJID, twitterID) in AOJIDs:
        if AOJID in acCount:
            if nowACCount[AOJID] - acCount[AOJID] > 0:
                newACCount.append(({"AOJID" : AOJID, "twitterID" : twitterID, "count" : nowACCount[AOJID] - acCount[AOJID]}))
    ...
    for idx in range(len(newACCount)):
        ...
        if countRankNum <= 5:
            ...
            countRankingDraw.text((120, 7), newACCount[idx]["AOJID"], fill = (0, 0, 0), font = rankingFontS)
            ...
            countRankingDraw.text((120, 7), newACCount[idx]["AOJID"], fill = (0, 0, 0), font = rankingFont)
    ...
    api.update_staus_with_media(filename = "AOJ/data/countRankingImg_fixed.jpg", status = countTweetText + "\n" + timeStamp)
