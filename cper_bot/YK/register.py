...
def checkYKID(ykID):
    ...
    try:
        ...
        print("YK-register: " + ykID + " is correct yukicoder ID")
        return True
    except:
        print("YK-register: " + ykID + " is not correct yukicoder ID")
        return False
...
def downloadFromDropbox():
    ...
    print("YK-register: Downloaded YKID (size : ", str(len(YKID)), ")")
    ...
    print("YK-register: Downloaded YK_TwitterID (size : ", str(len(YK_TwitterID)), ")")
...
def uploadToDropbox():
    ...
    if YK_listFixedFlag:
        ...
        with open("YK/YKID.txt", "rb") as f:
            ...
            print("YK-register: Uploaded YKID (size : ", str(len(YKID)), ")")
        with open("YK/YKID.txt", "rb") as f:
            dbx.files_upload(f.read(), "/_backup/YK/YKID/" + str(datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S")) + ".txt")
            print("YK-register: Uploaded YKID (for backup " + str(datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S")) + ") (size : ", str(len(YKID)), ")")
        ...
        with open("YK/TwitterID.txt", "rb") as f:
            ...
            print("YK-register: Uploaded YK_TwitterID (size : ", str(len(YK_TwitterID)), ")")
        with open("YK/TwitterID.txt", "rb") as f:
            dbx.files_upload(f.read(), "/_backup/YK/TwitterID/" + str(datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S")) + ".txt")
            print("YK-register: Uploaded YK_TwitterID (for backup " + str(datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S")) + ") (size : ", str(len(YK_TwitterID)), ")")
...
def register(ykID, twitterID, flag):
    ...
    print("YK-register: ----- YK-register Start -----")
    ...
    if flag == 0:
        if checkYKID(ykID):
            ...
            print("YK-register: Registered new YK ID : " + ykID)
            YK_listFixedFlag = True
        else:
            tweetText = "@" + str(twitterID) + " 正しい yukicoder ID ではありません！\n"
            print("YK-register: Rejected to YK-register new YK ID : " + ykID)
    if flag == 1:
        if checkYKID(ykID):
            if myIndex(ykID, YKID) != -1 and myIndex(str(twitterID), YK_TwitterID) != -1 and myIndex(ykID, YKID) == myIndex(str(twitterID), YK_TwitterID):
                ...
                print("YK-register: Unregistered YK ID : " + ykID)
                YK_listFixedFlag = True
            else:
                tweetText = "@" + str(twitterID) + " この yukicoder ID は登録されていません！\n"
                print("YK-register: Rejected to unregister YK ID : " + ykID)
        else:
            tweetText = "@" + str(twitterID) + " 正しい yukicoder ID ではありません！\n"
            print("YK-register: Rejected to unregister YK ID : " + ykID)
    ...
    print("YK-register: ----- YK-register End -----")
    return tweetText
