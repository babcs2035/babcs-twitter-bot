...
def contest():
    ...
    contestsJsonRes = urllib.request.urlopen(
        "https://codeforces.com/api/contest.list?lang=us")
    ...
    if len(contestsList) == 0:
        api.update_status(
            "現在予定されている Codeforces コンテストはありません．\nhttps://codeforces.com/contests\n\n" + timeStamp)
        return

    # 画像生成
    listFont = ImageFont.truetype("CF/data/fontR.ttc", 32)
    contestsListFirstImg = Image.open(
    ...
    for contest in contestsList:
        contestListImg = Image.open(
            "CF/data/contest/contestsListImg (cell).jpg")
        contestListDraw = ImageDraw.Draw(contestListImg)
        contestListDraw.text((10, 7), str(epoch_to_datetime(
            contest["startTimeSeconds"])), fill=(0, 0, 0), font=listFont)
        contestListDraw.text((360, 7), str(
            contest["name"]), fill=(0, 0, 0), font=listFont)
        contestListDraw.text((1460, 7), str(sec_to_time(
            contest["durationSeconds"])), fill=(0, 0, 0), font=listFont)
        contestListDraw.text((1660, 7), str(
            contest["type"]), fill=(0, 0, 0), font=listFont)
    ...
    # リストをツイート
    listTweetText = "現在 " + \
        str(idx) + " の Codeforces コンテストが予定されています．\nhttps://codeforces.com/contests\n"
    api.update_status_with_media(
        filename="CF/data/contest/contestsListImg_fixed.jpg", status=listTweetText + "\n" + timeStamp)
